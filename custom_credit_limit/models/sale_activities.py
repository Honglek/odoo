...
class SaleActivities(models.Model):
    ...
    def init(self):
        ...
        update_query_province = """
            UPDATE res_partner a
            SET province_name = b.name
            FROM kh_province b
            WHERE a.kh_province = b.id AND COALESCE(a.province_name, '') = '';
        """
        update_query_district = """
            UPDATE res_partner a
            SET district_name = b.name
            FROM kh_district b
            WHERE a.kh_district = b.id AND COALESCE(a.district_name, '') = '';
        """
        
        self.env.cr.execute(update_query_province)
        self.env.cr.execute(update_query_district)
